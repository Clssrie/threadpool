...
#define MIN_WAIT_TASK_NUM 10		//如果queue_size > MIN_WAIT_TASK_MUN添加新的线程到线程池
...
threadpool_t *threadpool_create(int min_thr_num, int max_thr_num, int queue_max_size)
{
	...
	do
	{
		...
		{
			printf("malloc taskk queue fail\n");
			break;
		}
		...
		{
			printf("init the locks or conds fail\n");
			break;
		}
...
void *threadpool_thread(void *threadpool)
{
	...
	while (true)
	{
		//刚创建出线程，等待任务队列里有任务，否则阻塞等待任务队列里有任务
		//后在唤醒接受任务
		pthread_mutex_lock(&(pool->lock));

		//queue_size = 0 说明没有任务，调wait阻塞在条件变量上，若有任务，跳过
...
void* adjust_thread(void* threadpool)
{
	...
	while (!pool->shutdown)
	{
		...
		if (queue_size >= MIN_WAIT_TASK_NUM && live_thr_num < pool->max_thr_num)
		{
			...
			for (i = 0; i < pool->max_thr_num && DEFAULT_THREAD_VARY &&
		...
		//销毁多余的空闲线程 算法：忙线程*2 小于 存活的线程数且存活的线程数大于最小线程数时
...
void* threadpool_add(threadpool_t *thp, void*(*function)(void* arg), void* arg)
{
	threadpool_t* pool = thp;
	pthread_mutex_lock(&(pool->lock));
	...
	pool->queue_size++; //队尾指针移动，模拟环形

	//添加任务后，队里不为空，唤醒线程池中等待处理任务的线程
	pthread_cond_signal(&(pool->queue_not_empty));
	pthread_mutex_unlock(&(pool->lock));
